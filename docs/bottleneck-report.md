...
This report documents the performance characteristics, bottlenecks, and optimization results for the Snip URL Shortener running on a DigitalOcean s-2vcpu-4gb droplet (2 vCPUs, 4GB RAM, 2GB swap).
...
At Gold tier, total CPU demand exceeds the 200% available from 2 vCPUs (100% per core), confirming CPU as the primary bottleneck. The kernel scheduler time-slices between processes, adding latency.
...
Memory usage stays well within the 4GB RAM + 2GB swap capacity. No OOM kills observed during testing. PostgreSQL memory growth under load is expected (shared buffers, sort operations, connection state).
...
With `gthread` workers (4 workers x 2 threads per instance), the thread pool reuses connections effectively. Connection count only spikes under extreme load when threads block waiting for I/O.
...
| Concurrent handlers | 8 (4w x 2t) | 24 (3i x 4w x 2t) | 3x increase |
...
| Metric | Before (sync workers) | After (gthread, 4w x 2t) | Improvement |
|--------|----------------------|---------------------------|-------------|
| Concurrent handlers/instance | 4 | 8 | 2x increase |
...
1. **Upgrade Droplet** to 4 vCPU / 8GB ($48/mo) -- doubles CPU capacity, projected to handle 800+ VU under p95 < 5s
...
|---------------|-------------|------------------------------|---------------------|
| Current (s-2vcpu-4gb, 3 instances) | $24 | ~550 | $24 |
| Upgrade to s-4vcpu-8gb | $48 | ~1,100 | $24 |
| 2x s-2vcpu-4gb + DO LB | $60 | ~1,800 | $17 |
| s-4vcpu-8gb + PgBouncer | $48 | ~1,500 | $16 |
